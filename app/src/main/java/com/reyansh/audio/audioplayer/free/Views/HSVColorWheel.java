...
public class HSVColorWheel extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (bitmap != null) {
            canvas.drawBitmap(getCircularBitmapWithWhiteBorder(bitmap,1), null, rect, null);
            float hueInPiInterval = colorHsv[0] / 180f * (float) Math.PI;

            selectedPoint.x = rect.left + (int) (-(float) Math.cos(hueInPiInterval) * colorHsv[1] *
            ...
            canvas.drawLine(selectedPoint.x - pointerLength,
                    selectedPoint.y,
                    selectedPoint.x + pointerLength, selectedPoint.y,
                    pointerPaint);
            canvas.drawLine(selectedPoint.x,
                    selectedPoint.y - pointerLength,
                    selectedPoint.x, selectedPoint.y + pointerLength,
                    pointerPaint);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (listener != null) {
                    listener.selectedColor(getColorForPoint((int) event.getX(), (int) event.getY(), colorHsv));
                }
                Log.d("color", String.valueOf(getColorForPoint((int) event.getX(), (int) event.getY(), colorHsv)));
